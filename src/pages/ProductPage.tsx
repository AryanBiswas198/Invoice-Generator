...
const ProductPage: React.FC = () => {
        ...
        <div key={product.id} className="rounded-lg shadow-md p-4 mb-4">
          <div className="flex flex-col md:flex-row items-center gap-4 md:gap-10 border-t-2 pt-8 border-green-50">
            <img
              src={product.image}
              alt={product.name}
              className="w-40 h-40 md:w-48 md:h-auto object-cover rounded-md mr-4"
            />
            <div className="flex flex-col justify-between flex-grow">
              <div>
                ...
                <p className="text-gray-300 md:text-lg mb-3">
                  Price: ${product.price}
                </p>
              </div>
  ...
  const renderCart = () => {
    const selectedProductDetails = products.filter(
      (product) => selectedProducts[product.id] !== undefined
    );
    const totalRate = selectedProductDetails.reduce(
      (sum, product) => sum + product.price * selectedProducts[product.id],
      0
    );
    const totalGST = totalRate * 0.18;
    ...
    return (
      <div className="rounded-lg shadow-md p-4 md:p-14 w-full lg:w-2/4">
        ...
        {selectedProductDetails.length === 0 ? (
          <p className="text-red-500 font-extrabold text-center my-8 text-3xl md:text-5xl">
            Your Cart is Empty
          </p>
        ) : (
            ...
            >
              <div className="flex items-center mb-3 md:mb-0">
                ...
                <div className="flex flex-col">
                  ...
                  <p className="text-gray-300 tracking-wider md:text-lg my-1">
                    Price: ${product.price}
                  </p>
                </div>
              </div>
        ...
        <div className="mt-8 md:mt-32 flex flex-col">
          <p className="text-3xl md:text-4xl text-gray-300 my-2 font-bold">
            Total:{" "}
            <span className="text-2xl md:text-3xl text-gray-100">
              ${totalRate.toFixed(2)}
            </span>
          </p>
          <p className="text-xl md:text-2xl font-bold my-2 text-gray-300">
            GST: ${totalGST.toFixed(2)}
          </p>
          <p className="text-red-500 text-3xl md:text-4xl border-t-2 py-2 my-4 font-semibold">
